...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        rgb_floats = matrix_multiply(color_matrix, pixel)  # list of floats
        rgb_ints = tuple(int(v) for v in rgb_floats)     # tuple of ints
        new_px.append(rgb_ints)
    
    return new_px
    # new_px = []
    # color_matrix = make_matrix(color)
    # for pixel in pixels_list:
    #     new_px.append(tuple(matrix_multiply(color_matrix, pixel)))
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if isinstance(pixel, int):
        ...
        return tuple(val % (2**num_end_bits) for val in pixel)
...
def reveal_bw_image(filename):
    ...
    for pixel in px:
        ...
        new_px.append(hidden * 255)
    return pix_to_img(new_px, (width, length), 'L')
...
def reveal_color_image(filename):
    ...
    new_px = []
    for pixel in px:
        hidden = extract_end_bits(3, pixel)
        new_px.append(tuple(val * (255//7) for val in hidden))
    return pix_to_img(new_px, (width, length), 'RGB')
...
def main():
    ...
    # im2.show()
    draw_kerb('unhidden1.png','jamest')
